...
- [x] **2026-06-30 — Fix: selektor Deep Dive mieszał audyty do „latest score"**. Objaw: materiał (Income Statement, wcześniej zaliczony) pokazywał się jako aktywny „X śr · 1 poniżej 6", ale runda dawała „Brak pytań otwartych". Przyczyna: `getLatestScoresForItems` + 4 zapytania statystyk w `app/(app)/sessions/deep-dive/page.tsx` nie filtrowały `is_audit = false` — w przeciwieństwie do kolejki rundy (`getLatestReviewByItem` w `start/route.ts`) i licznika nawigacji (`countUnmasteredOpen` w `lib/db/counts.ts`), które filtrują. Audytowy niski score (np. samoocena „Pustka"/„Mgliście" <6) był brany jako ostatni score pytania otwartego → fałszywa luka w selektorze. Fix: dodane `.eq("is_audit", false)` w 5 miejscach selektora. Regres recall z audytu należy do toru `decayed_mastery` (5. detektor), nie do Deep Dive. Build green (tsc 0, eslint 0).
...
- [x] **Deploy detektora `decayed_mastery` na prod** (2026-06-30, commit `06629a5` → Vercel). Potwierdzone przez użytkownika: działa.
